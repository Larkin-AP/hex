...

{
	...
	cs.resetMaster(robot::createMasterROSHexTest().release());
	cs.resetController(robot::createControllerROSHexTest().release());
	cs.resetPlanRoot(robot::createPlanROSHexTest().release());
	...
	dynamic_cast<aris::dynamic::AdamsSimulator&>(adams1).saveAdams("C:\\Users\\jpche\\Desktop\\hex\\hexapod_simulation_with_control.cmd", dynamic_cast<aris::dynamic::Model&>(cs.model()).simResultPool().front()); //这个模型还含有运动参数
